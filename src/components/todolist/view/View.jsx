...
const View = () => {
  ...
  return (
    <div className="flex gap-3 flex-wrap flex-row justify-center text-center w-[50%] h-full">
      {userData?.map((user) => (
        <div
          className="p-3 flex flex-col justify-start text-start bg-slate-100 shadow-lg shadow-slate-500/40  rounded-xl w-[270px]"key={user.id}
        >
          ...
          <p className="text-xl text-gray-700 font-mono border  rounded-2xl p-[4px]">
            <span className="text-gray-500 font-mono">Bio:</span>
            {user.bio}
          </p>
        </div>
      ))}
